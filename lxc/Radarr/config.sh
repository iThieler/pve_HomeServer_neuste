...
ghTag=$(githubLatest Radarr/Radarr)
...
pct exec $ctID -- bash -ci "wget -q https://github.com/Radarr/Radarr/releases/download/${ghTag}/Radarr.master.$(echo ${ghTag} | sed 's#v##').linux.tar.gz"
pct exec $ctID -- bash -ci "tar -xvzf Radarr.$(echo ${ghTag} | sed 's#v##').linux.tar.gz > /dev/null 2>&1"
